...
- When the session cannot stop and wait (`codex exec`, a scheduled task, a subagent), never block: take the most defensible reading, proceed, and lead the report with the assumptions made and what changes under the alternatives. Interactively, a slow reply is never a reason to skip the question.

## Sandbox and approvals

- Work inside the sandbox. Request escalation only when the task genuinely needs it (network install, writes outside the workspace), and say in one line why.
- Never work around a denied command by rewriting it into a form that dodges the rule; a denial means change course, not disguise.
...
- Temp files, one-off scripts, fixtures, downloads, plan files, and task worktrees go in a temp directory (`$TMPDIR` or `~/.codex/tmp`), never loose in the repo or scattered through the home directory; remove them when the task is done.
